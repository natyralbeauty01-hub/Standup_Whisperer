...
function buildPrompt(rawNotes: string, format: string): { system: string; user: string } {
  ...
  if (noteLen < 50) activeEdgeCases.push(EDGE_CASE_HANDLERS.tooShort);
  if (noteLen > 500) activeEdgeCases.push(EDGE_CASE_HANDLERS.tooLong);
  ...
  if (futureSignals > 3 && pastSignals === 0) activeEdgeCases.push(EDGE_CASE_HANDLERS.futureFocused);

  const namePattern = /\b(?:with|from|on|waiting for|pending)\s+([A-Z][a-z]+)\b/g;
  if (namePattern.test(rawNotes)) activeEdgeCases.push(EDGE_CASE_HANDLERS.containsNames);

  if (/\b(yesterday|today|blocker)\b/i.test(rawNotes)) activeEdgeCases.push(EDGE_CASE_HANDLERS.alreadyStructured);
...
router.post("/standup/generate", async (req, res): Promise<void> => {
  ...
  try {
    ...
    req.log.error({ err }, "Error streaming standup generation");
